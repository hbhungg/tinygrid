...
from .schedule_data_model import Schedule_Model
...
class IEEE_CISMixin:
  ...
  # NOTES: fix this cause of the breaking changes
  ...
  def _load_energy_data(cls) -> dict[str, pd.DataFrame]:
    ...
    """
    energy_data_path = os.path.join(cls._BASE_DIR, "energy/nov_data.tsf")
    tsf_file = convert_tsf_to_dataframe(energy_data_path)[0]
    data = dict()
  ...
  def _load_ERA5_weather_data(cls) -> pd.DataFrame:
    ...
    Return:
      DataFrame object
    """
    weather_data_path = os.path.join(cls._BASE_DIR, "weather/ERA5_Weather_Data_Monash.csv")
  ...
  def _load_AEMO_nov_price_data(cls) -> pd.DataFrame:
    ...
    Return:
      DataFrame object
    """
    nov_price_data_path = os.path.join(cls._BASE_DIR, "price/PRICE_AND_DEMAND_202011_VIC1.csv")
    price_data = pd.read_csv(nov_price_data_path)
    # Parse the time from string to 
  ...
  @classmethod
  def _get_phase_time(cls) -> tuple:
    return (cls.PHASE1_TIME, cls.PHASE2_TIME)
